...
export default function CariKosClient() {
  ...
  const filtered = kosList.filter((kos) => {
    ...
    if (onlyAvailable && !kos.kamar.some((k) => k.ketersediaan === "Tersedia")) return false;
    return true;
  });
    ...
    <>
      <div className="mt-6 rounded-2xl bg-surface p-5 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <span className="text-xs text-text-secondary">Menampilkan {filtered.length} dari {kosList.length} kos</span>
        </div>
      </div>

      <div className="mt-10">
        <div className="mt-5 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
          {filtered.map((kos) => <KosCard key={kos.id} id={kos.id} nama={kos.nama} lokasi={kos.lokasi} harga={kos.harga} gambar={kos.gambar} tipe={kos.tipe} status={kos.status} />)}
        </div>
