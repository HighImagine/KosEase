...
import { deleteKosAction, setPublikasiAction } from "@/app/kos/actions";
...
export function PublikasiToggle({ kosId, status }: { kosId: string; status: StatusPublikasi }) {
  ...
  return (
    <form action={formAction} className="inline">
      ...
      >
        {status === "tayang" ? "Jadikan Draft" : "Tayangkan"}
      </button>
    </form>
  );
}
...
export function DeleteKosButton({ kosId }: { kosId: string }) {
  return (
    <form
      ...
      className="inline"
      onSubmit={(e) => {
        if (!window.confirm("Hapus kos ini beserta kamar, foto, dan fasilitasnya? Tindakan tidak bisa dibatalkan.")) {
          e.preventDefault();
        }
      }}
    >
