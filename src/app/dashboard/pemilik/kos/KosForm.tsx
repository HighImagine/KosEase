...
export default function KosForm({ kos, action }: { kos?: Kos | null; action: KosAction }) {
  ...
  return (
    <form action={formAction} className="space-y-4">
      ...
      <div>
        <label htmlFor="nama" className={labelCls}>Nama Kos</label>
        <input id="nama" name="nama" required defaultValue={kos?.nama ?? ""} placeholder="cth: Kos Melati" className={inputCls} />
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="tipe" className={labelCls}>Tipe Kos</label>
        ...
        <div>
          <label htmlFor="lokasi" className={labelCls}>Lokasi (Kota)</label>
          <input id="lokasi" name="lokasi" required defaultValue={kos?.lokasi ?? ""} placeholder="cth: Yogyakarta" className={inputCls} />
        </div>
      </div>

      <div>
        <label htmlFor="alamat" className={labelCls}>Alamat Lengkap</label>
        <input id="alamat" name="alamat" required defaultValue={kos?.alamat ?? ""} placeholder="cth: Jl. Kaliurang KM 5, Sleman" className={inputCls} />
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor="harga" className={labelCls}>Harga Termurah / Bulan (Rp)</label>
          <input id="harga" name="harga" type="number" min={0} step={50000} required defaultValue={kos?.harga ?? 0} className={inputCls} />
          <p className="mt-1 text-[10px] text-text-secondary">Otomatis ikut harga kamar termurah saat kamar diubah.</p>
        </div>
        <div>
          <label htmlFor="status_publikasi" className={labelCls}>Status Publikasi</label>
          <select id="status_publikasi" name="status_publikasi" defaultValue={kos?.status_publikasi ?? "draft"} className={inputCls}>
            <option value="draft">Draft (tidak tampil publik)</option>
            <option value="tayang">Tayang (tampil publik)</option>
          </select>
        </div>
      </div>
      ...
      <div>
        ...
        <textarea id="deskripsi" name="deskripsi" rows={4} defaultValue={kos?.deskripsi ?? ""} placeholder="Ceritakan keunggulan kos..." className={`${inputCls} resize-none`} />
      </div>
      ...
      <button type="submit" disabled={pending} className="flex w-full items-center justify-center rounded-lg bg-primary py-2.5 text-xs font-semibold text-white transition-colors hover:bg-primary-dark disabled:opacity-50">
        {pending ? "Menyimpan..." : kos ? "Simpan Perubahan" : "Buat Kos"}
      </button>
    </form>
  );
}
