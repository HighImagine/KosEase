...
import { z } from "zod";
// redirect polos next/navigation: path SELALU sudah berprefix via withLocale(),
// karena redirect next-intl tidak dijamin di dalam Server Actions.
...
export async function registerAction(_prev: unknown, formData: FormData) {
  ...
  redirect(withLocale("/login?registered=1", formLocale(formData)));
}
...
export async function loginAction(_prev: unknown, formData: FormData) {
  ...
  if (next) redirect(withLocale(next, locale));
...
const norm = role?.toLowerCase();
    if (norm === "pemilik") redirect(withLocale("/dashboard", locale));
    if (norm === "pemilik_kos") redirect(withLocale("/dashboard", locale));
    if (norm === "admin") redirect(withLocale("/dashboard/admin", locale));
    // penyewa/ -> homepage
...
export async function adminLoginAction(_prev: unknown, formData: FormData) {
  ...
  redirect(withLocale("/dashboard/admin", formLocale(formData)));
}
...
export async function resetPasswordAction(_prev: unknown, formData: FormData) {
  ...
  if (error) return { error: error.message };
  redirect(withLocale("/login?reset=1", formLocale(formData)));
}
...
export async function pengajuanPemilikAction(_prev: unknown, formData: FormData) {
  ...
  redirect(withLocale("/dashboard/pengajuan-pemilik?success=1", formLocale(formData)));
}
...
export async function approvePengajuanAction(_prev: unknown, formData: FormData) {
  ...
  if (error) return { error: error.message };
  redirect(withLocale("/dashboard/admin/pengajuan", formLocale(formData)));
}
